...
enum StaffAuthMode { register, login }

class StaffRegisterScreen extends StatefulWidget {
  final StaffAuthMode mode;
  final String? prefillPhone;

  const StaffRegisterScreen({
    super.key,
    this.mode = StaffAuthMode.register,
    this.prefillPhone,
  });
...
class _StaffRegisterScreenState extends State<StaffRegisterScreen> {
  ...
  bool get isLoginMode => widget.mode == StaffAuthMode.login;

  @override
  void initState() {
    super.initState();
    // Prefill phone when coming from login
  ...
  Future<void> _selectDate(BuildContext context) async {
    if (isLoginMode) return; // not needed in login mode
