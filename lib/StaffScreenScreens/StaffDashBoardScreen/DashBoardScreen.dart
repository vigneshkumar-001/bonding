...
class _BondingDashboardPageState extends State<BondingDashboardPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (staff != null && staff.memberID.isNotEmpty) {
        final userId = staff.memberID;
        final userName = _safeName(staff.name, userId);
        final avatar = _safeAvatar(staff.image);

        debugPrint("Staff fetched → memberID: $userId name=$userName");

        // ✅ connect socket
        socketService.connectStaff(userId);

        // ✅ Connect ZIMKit (chat) with safeName
        ...
        // ✅ Init / Switch Zego properly (ONLY ONCE)
        await _initOrSwitchZego(userId: userId, userName: userName);
      }
    });
  }

  // ✅ Optional: keep if you use elsewhere
  Future<void> _connectZimUser(StaffSingleProfile? staff) async {
    ...
    try {
      await ZIMKit().connectUser(id: userId, name: userName, avatarUrl: avatar);
      if (mounted) setState(() => _isZimConnected = true);
      debugPrint("→ ZIMKit login SUCCESS for staff: $userId");
    } catch (e) {
  ...
  // ✅ Correct Zego init for staff switch
  Future<void> _initOrSwitchZego({
    required String userId,
    required String userName,
  }) async {
    // same user already init
    if (_zegoInitialized && _zegoUserId == userId) {
      debugPrint("Zego already initialized for: $userId");
      return;
    }

    // different staff logged in → must uninit
    if (_zegoInitialized && _zegoUserId != null && _zegoUserId != userId) {
      debugPrint(
        "Switching staff: $_zegoUserId -> $userId, uninit old Zego...",
      );
      try {
    ...
    debugPrint("✅ Zego initialized for staff: $userId name=$safeUserName");
  }
