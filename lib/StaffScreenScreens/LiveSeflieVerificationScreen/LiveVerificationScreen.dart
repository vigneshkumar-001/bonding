...
class _LiveVerificationScreenState extends State<LiveVerificationScreen> {
  ...
  Future<bool> _requestCameraPermission() async {
    ...
    if (status.isPermanentlyDenied) {
      Utils.snackBarErrorMessage(
        "Camera permission blocked. Enable in settings.",
      );
  ...
  Future<bool> _requestGalleryPermission() async {
    ...
    if (Platform.isAndroid) {
      // Android 13+: photos (READ_MEDIA_IMAGES). Older devices may fallback to storage.
    ...
    if (status.isPermanentlyDenied) {
      Utils.snackBarErrorMessage(
        "Gallery permission blocked. Enable in settings.",
      );
      await _showPermissionSettingsDialog(
